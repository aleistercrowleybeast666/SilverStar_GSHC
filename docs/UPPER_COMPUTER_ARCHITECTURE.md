...
本文描述 SilverStar 0.0.8 上位机的实时接收、状态、日志和 GUI 边界。AIR 字段以 [AIR_PROTOCOL.md](AIR_PROTOCOL.md) 为准，GSP wire format 以 [GSP_MIN_PROTOCOL.md](GSP_MIN_PROTOCOL.md) 为准。
...
Persistent Recording
  ...
  PREFLIGHT_STATE、STATUS、ACK、FLIGHT_STATE
  -> 整个飞控会话
...
- Alignment state/source ready/overall ready；
...
```text
预飞行：System / Calibration / Alignment / GNSS / PREFLIGHT_STATE / commands
飞行：重要状态 / 当前数据 / EventHistory / 6 plots
